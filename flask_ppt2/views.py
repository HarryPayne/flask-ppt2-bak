...
def getAllAttributesJSON():
    """Send getAllAttributes response as JSON."""
...
def getAttributesFromForm(form):
    ...
    # This method is on its way out. In the meantime, translate from
    # formly attributes into the legacy format.
    attributes = {}
    # attributes not in the form:
    attributes["csrf_token"] = {"name": "csrf_token",
                                "format": "hidden"}
...
def getFormatFromField(field):
    ...
    """
    if field["type"] in ["QuerySelectMultipleField", "QuerySelectField"]:
        return "multipleSelect"
    elif field["type"] == "IntegerField":
        return "number"
    elif  field["key"].endswith("InY") or field["key"].endswith("InFY"):
        return "dateRangeSelect"
    elif field["key"].endswith("InQ") or field["key"].endswith("InM"):
        root = field["key"][:field["key"].index("In")]
...
def getMultiFromField(field):
    ...
    """
    if field["type"] in ["QuerySelectMultipleField"]:
        return True
    else:
        return False
